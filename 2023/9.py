...
class SensorDataCollection:
    def __init__(self, data: List[int]) -> None:
        ...
        while not self.calculations[-1].check_for_all_zero():
            self.calculations.append(SensorData(self.calculations[-1].get_differences()))
            
    def calculate_extrapolations(self):
        for i in range(len(self.calculations) - 1, 0, -1):
            self.calculations[i].calculate_extrapolation()
            self.calculations[i - 1].add_extrapolation(self.calculations[i].get_extrapolation_value())
...
def main(data: List[str], reverse: bool = False):
    ...
    for line in data:
        if not reverse:
            ...
            sensor_data.append(SensorDataCollection([int(x) for x in line.split(" ")][::-1]))
    ...
    for sensor in sensor_data:
        ...
        print(sensor)
        print(sensor.get_extrapolation())
